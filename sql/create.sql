...
INSERT INTO `test`.`mock_config`(`id`, `uri`, `method`, `type`, `sync_response`, `async_response`, `async_call_path`, `async_method`, `delay_seconds`, `response_content_type`, `async_http_header`) VALUES (1, '/test', 'GET', 0, '{"method": "get", "helloWorld": {{query:test}}}', NULL, NULL, NULL, NULL, 'application/json;charset=UTF-8', '');
INSERT INTO `test`.`mock_config`(`id`, `uri`, `method`, `type`, `sync_response`, `async_response`, `async_call_path`, `async_method`, `delay_seconds`, `response_content_type`, `async_http_header`) VALUES (2, '/test', 'POST', 1, '{"method": "post", "helloWorld": {{body:helloWorld}}, "randomString": {{UUID}}, "randomNumber": {{RANDOM_NUMBER}}, "inner":{{body:testObj:objObj:innerData}}}', '{"response": "test", "testText":{{body:testText}}}', 'http://localhost:9099/test', 'GET', 10, 'application/json;charset=UTF-8', '{"Content-Type": "application/json;charset=UTF-8", "Authorization": "ODg5NGFmMjAtZmFhZC00ZWU3LWJkZWQtMjE1MzExOWFhYmFkLjE1ODI1ODE4Nzk3ODY="}');